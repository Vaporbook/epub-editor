...
module.exports.init = function init(zipfile, cb) {
	parser.open(zipfile, function (err, epub) {
		...
		cb(null,{
			...
			dom: {
				getOpf: function () { return opfDom; },
				setOpf: function (newdom) { opfDom = newdom },
				getNcx: function () { return ncxDom; },
				setNcx: function (newdom) { ncxDom = newdom }
			},
...
function writeChanges() {
	...
	fs.writeFile(filename, zipContainer.generate({base64:false,compression:'DEFLATE'}), 'binary', function(err) {
		if(err) throw(err);
	});
